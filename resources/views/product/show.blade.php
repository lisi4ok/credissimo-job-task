           ...
           <div class="col-xs-12">
               ...
               <div class="table-responsive m-t-20">
                   <table class="table">
                        ...
                        @if ($productAttributeValue->attribute->field_type == 'SELECT')
                           <tr>
                               <td width="400">{{ $productAttributeValue->attribute->name }}</td>
                               <td>
                                   @php
                                        $attributeDropdownOption = $productAttributeValue
                                            ->attribute
                                            ->attributeDropdownOptions()
                                            ->where('id', $productAttributeValue->value)->first();
                                            $displayText = (isset($attributeDropdownOption->display_text)) ? $attributeDropdownOption->display_text : NULL;
                                   @endphp
                                   {{ $displayText }}
                               </td>
                           </tr>
                        @elseif ($productAttributeValue->attribute->field_type == 'TEXT')
                           <tr>
                               <td width="400">{{ $productAttributeValue->attribute->name }}</td>
                               <td>
